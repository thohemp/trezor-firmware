...
static const uint8_t WIPE_CODE_EMPTY[] = {0};
static const size_t WIPE_CODE_EMPTY_LEN = 1;
...
static secbool set_wipe_code(const uint8_t *wipe_code, size_t wipe_code_len) {
  if (wipe_code_len == 0) {
    // This is to avoid having to check pin != "" when checking the wipe code.
...
static secbool storage_upgrade(void) {
  ...
  if (norcow_active_version == 0) {
    ...
    if (sectrue == found && *(const uint32_t *)val != V0_EMPTY_PIN) {
      set_pin((const uint8_t *)val, len, NULL);
    } else {
      set_pin((const uint8_t *)&V0_EMPTY_PIN, sizeof(V0_EMPTY_PIN), NULL);
      ret = norcow_set(PIN_NOT_SET_KEY, &TRUE_BYTE, sizeof(TRUE_BYTE));
    }
